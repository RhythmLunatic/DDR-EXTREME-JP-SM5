	...
	local t = {
		...
		LoadSelections = function(self, list, pn)
			...
			if filterValue ~= nil then
				...
				ActiveModifiers[pName]["ScreenFilter"] = 0
				list[1] = true
			end
		end,
		SaveSelections = function(self, list, pn)
			...
			for i=1,#list do
				if not found then
					if list[i] == true then
						ActiveModifiers[pName]["ScreenFilter"] = choiceToAlpha[i]
						found = true
					end
				end
			end
...
--Stolen from Simply Love.
--[[function OptionRowMusicRate()
	local MusicRate = {
		Choices = function()
			local first	= 0.05
			local last 	= 2
	...
	setmetatable(MusicRate, MusicRate)
	return MusicRate
end]]
