...
if highscores then
	...
	for s=lower,upper do
		...
		row[#row+1] = LoadFont("MusicScroll Titles")..{
			Text=s..". ",
		...
		row[#row+1] = LoadFont("MusicScroll Titles")..{
			Text=score,
			InitCommand=cmd(horizalign,right; xy, -15, (s-(lower-1))*row_height )
		}

		row[#row+1] = LoadFont("MusicScroll Titles")..{
			Text=date,
			InitCommand=cmd(horizalign,left; xy, 0, (s-(lower-1))*row_height )
		}
